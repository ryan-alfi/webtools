<?php 
    ...
    if ( null==$id ) {
        ...
        $sql = "SELECT * FROM tmentahan where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-8">
				...
				<p><?php 
						 // $txt = $data['isi'];
						$txt = "Bukan karena hanya bisa. tapi itu/this bukan lah (12/6) hal yang wajar. maka dari itu waspadalah";
						$KArtikel = $data['kategori'];
						echo $txt; ?></p>
				</div>
				<div class="atas">
				<h3>Splitted</h3>
...
?>
				<form name="myForm" action="input_splitted.php" method="post" onsubmit="validateForm()">
					...
					<div class="bawah"><br>
					<h3>Joinned</h3>
					<a class="btn btn-primary" id='btn-tambah' >Tambah Frasa</A>
	...
	$(document).ready(function(){
		$("#btn-tambah").on("click",function(){
			...
			var comp = '<tr id=\'baris-'+idnyabaris+'\'>'+
						'<td><input type="text"  name="gab[]" onClick=\'klik('+idnyabaris+')\' class=\'gabungan\' id="gabungan-'+idnyabaris+'" onfocus="focusFunction(this)" onblur="blurFunction(this)" value="" ></td>'+
						'<td>'+
...
*/
		...
		function focusFunction(r) {
		    r.style.background = "yellow";
		}

		function blurFunction(i) {
		    i.style.background = "#09FF00";
		}
